const SplashPage = window.SplashPage = ({ onNavigate, inviteInfo }) => {
  ...
  const [wtStep, setWtStep] = React.useState(0);
  const [wtTyping, setWtTyping] = React.useState(false);
  const [wtTypedText, setWtTypedText] = React.useState('');
  ...
  return (
    <div className="splash-page">
      ...
      <section style={{ maxWidth: '960px', margin: '0 auto', padding: '64px 24px' }}>
        <h2 style={{ textAlign: 'center', fontSize: '32px', color: '#1b6b5a', marginBottom: '8px' }}>Built for Everyone in the Care Circle</h2>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginBottom: '32px', flexWrap: 'wrap' }}>
          {[
            ...
            <button key={tab.id} onClick={() => switchTab(tab.id)} style={{
              ...
              background: activeTab === tab.id ? '#1b6b5a' : '#fff',
        ...
        {activeTab === 'families' && (
          <div>
            <div style={{ textAlign: 'center', marginBottom: '32px' }}>
              <h3 style={{ fontSize: '24px', color: '#333', marginBottom: '8px' }}>You Shouldn't Have to Do This Alone</h3>
              <p style={{ fontSize: '15px', color: '#666', maxWidth: '600px', margin: '0 auto', lineHeight: 1.6 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '16px' }}>
              {[
                ...
                <div key={i} style={{ padding: '24px', background: '#fff', borderRadius: '12px', border: '1px solid #e8e8e8', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                  <div style={{ fontSize: '24px', marginBottom: '10px' }}>{c.icon}</div>
                  <div style={{ fontSize: '15px', fontWeight: 600, color: '#1b6b5a', marginBottom: '6px' }}>{c.title}</div>
        ...
        {activeTab === 'recipients' && (
          <div>
            <div style={{ textAlign: 'center', marginBottom: '32px' }}>
              <h3 style={{ fontSize: '24px', color: '#333', marginBottom: '8px' }}>Stay Independent. Stay Home. Stay You.</h3>
              <p style={{ fontSize: '15px', color: '#666', maxWidth: '600px', margin: '0 auto', lineHeight: 1.6 }}>
        ...
        {activeTab === 'caregivers' && (
          <div>
            <div style={{ textAlign: 'center', marginBottom: '32px' }}>
              <h3 style={{ fontSize: '24px', color: '#333', marginBottom: '8px' }}>Fair Pay. Flexible Hours. Your Career.</h3>
              <p style={{ fontSize: '15px', color: '#666', maxWidth: '600px', margin: '0 auto', lineHeight: 1.6 }}>
            ...
            <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '24px' }}>
              {[
                ...
                <div key={i} style={{ flex: '1 1 140px', maxWidth: '180px', padding: '20px', background: '#f0faf8', borderRadius: '10px', textAlign: 'center', border: '1px solid #d0e8e3' }}>
                  <div style={{ fontSize: '28px', fontWeight: 700, color: '#1b6b5a' }}>{s.num}</div>
        ...
        {activeTab === 'students' && (
          <div>
            <div style={{ textAlign: 'center', marginBottom: '32px' }}>
              <h3 style={{ fontSize: '24px', color: '#333', marginBottom: '8px' }}>Earn Clinical Hours While Making a Difference</h3>
              <p style={{ fontSize: '15px', color: '#666', maxWidth: '600px', margin: '0 auto', lineHeight: 1.6 }}>
      ...
      {/* ── Interactive Walkthrough: A Care Team's Story ── */}
      {(() => {
        const steps = [
...
FLAGS: Ankle swelling trend + appetite decline warrant mention at upcoming podiatry visit.`;
...
This report was generated by inPlace AI from 30 days of structured caregiver observations. Data is encrypted and shared only with authorized care team members.`;
        const goToStep = (s) => {
          if (wtTimerRef.current) clearInterval(wtTimerRef.current);
          setWtTypedText('');
          setWtTyping(false);
          setWtStep(s);
        };
        const startTyping = (text, speed) => {
          ...
          setWtTyping(true);
          let i = 0;
          wtTimerRef.current = setInterval(() => {
            i++;
            setWtTypedText(text.slice(0, i));
            if (i >= text.length) {
              clearInterval(wtTimerRef.current);
              setWtTyping(false);
            }
          }, speed || 12);
        };
        ...
        const phoneFrame = (title, content) => (
          <div style={{
            ...
            border: '3px solid #222', boxShadow: '0 8px 40px rgba(0,0,0,0.12)',
        ...
        return (
          <section style={{ padding: '64px 24px', background: 'linear-gradient(180deg, #f8faf9 0%, #fff 100%)' }}>
            <div style={{ maxWidth: 900, margin: '0 auto' }}>
              <div style={{ textAlign: 'center', marginBottom: 12 }}>
                ...
                <h2 style={{ fontSize: 28, color: '#1b6b5a', marginBottom: 8 }}>How Small Notes Become Powerful Insights</h2>
                ...
                {steps.map((s, i) => (
                  <React.Fragment key={s.id}>
                    <button onClick={() => goToStep(i)} style={{
                      ...
                      background: wtStep === i ? '#1b6b5a' : wtStep > i ? '#e8f5e9' : '#f5f5f5',
                      color: wtStep === i ? '#fff' : wtStep > i ? '#2e7d32' : '#999',
                    ...
                    {i < steps.length - 1 && (
                      <div style={{ width: 20, height: 2, background: wtStep > i ? '#1b6b5a' : '#ddd', transition: 'background 0.3s', flexShrink: 0 }}></div>
                    )}
                  </React.Fragment>
                ))}
              </div>
              ...
              <div style={{ transition: 'opacity 0.3s' }}>
                {wtStep === 0 && phoneFrame('Elena\'s Profile', (
                  <div>
                ...
                {wtStep === 5 && (
                  <div style={{ maxWidth: 520, margin: '0 auto' }}>
                    ...
                    <div style={{
                      background: '#f8f9fa', borderRadius: 14, border: '1px solid #e0e0e0',
                      boxShadow: '0 4px 20px rgba(0,0,0,0.08)', overflow: 'hidden',
                    }}>
                ...
                {wtStep === 6 && (
                  <div style={{ maxWidth: 520, margin: '0 auto' }}>
                    ...
                    {/* Doctor's reply email */}
                    <div style={{
                      background: '#fff', borderRadius: 14, border: '1px solid #e0e0e0',
                      boxShadow: '0 4px 20px rgba(0,0,0,0.08)', overflow: 'hidden', marginBottom: 16,
                    }}>
                    ...
                    {/* Arrow showing it flowing into care record */}
                    <div style={{ textAlign: 'center', margin: '8px 0', fontSize: 20, color: '#1b6b5a' }}>{'\u2B07\uFE0F'}</div>
                    {/* Care record showing the doctor note */}
                    <div style={{
                      background: '#fff', borderRadius: 14, border: '2px solid #1b6b5a',
                      boxShadow: '0 4px 20px rgba(27,107,90,0.12)', overflow: 'hidden',
                    }}>
                      <div style={{ background: '#1b6b5a', color: '#fff', padding: '10px 16px', fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <section style={{ padding: '64px 32px', background: '#fff' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          <h2 style={{ fontSize: '28px', color: '#1b6b5a', textAlign: 'center', marginBottom: '40px' }}>How inPlace Works for Everyone</h2>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '32px', justifyContent: 'center' }}>
            <div style={{ padding: '36px', background: '#f0faf8', borderRadius: '12px', border: '1px solid #d0e8e3', flex: '1 1 280px', maxWidth: '420px' }}>
            ...
            <div style={{ padding: '36px', background: '#f0faf8', borderRadius: '12px', border: '1px solid #d0e8e3', flex: '1 1 280px', maxWidth: '420px' }}>
              <div style={{ fontSize: '16px', fontWeight: 600, color: '#1b6b5a', marginBottom: '12px' }}>Caregivers Keep 80%</div>
              <p style={{ fontSize: '15px', color: '#555', lineHeight: 1.6 }}>
