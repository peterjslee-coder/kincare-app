      ...
      <div>
        ...
        <div className="card" style={{ textAlign: 'center', padding: 40 }}>
          ...
          <button onClick={() => onNavigate && onNavigate('recipients')}
            style={{ padding: '10px 24px', background: '#1b6b5a', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>
            Add a Loved One
          </button>
        </div>
      </div>
          ...
          <div key={t.id} className="card" style={{ cursor: 'pointer', transition: 'box-shadow 0.15s' }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <span style={{ display: 'inline-block', marginTop: 6, fontSize: 12, fontWeight: 600, color: '#1b6b5a',
                  background: '#e0f2e9', padding: '2px 10px', borderRadius: 12, textTransform: 'capitalize' }}>{t.my_role}</span>
              </div>
