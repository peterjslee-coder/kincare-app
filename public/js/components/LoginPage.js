  ...
  return (
    <div className="login-container">
      <div className="login-card" style={cardStyle}>
        ...
        {appleAvailable && (
          <div style={{ marginBottom: 8 }}>
            <button onClick={() => { window.location.href = '/api/oauth/apple'; }} style={{ ...googleBtnStyle, background: '#000', color: '#fff', border: '1px solid #000' }} disabled={loading}>
            ...
            <div style={{ fontSize: 11, color: 'var(--text-muted)', textAlign: 'center', marginTop: 4 }}>
              When prompted, choose <strong>Share My Email</strong> to link your account.
            </div>
          </div>
        )}

        {/* Divider after social buttons but before passkey/email */}
        {(googleAvailable || appleAvailable) && !passkeyAvailable && (
        ...
        {/* Divider before email form when passkey is shown */}
        {passkeyAvailable && (
          <div style={{ position: 'relative', textAlign: 'center', margin: '16px 0' }}>
            <div style={{ borderTop: '1px solid #e0e0e0' }}></div>
            <span style={{ position: 'relative', top: '-10px', background: 'var(--bg-surface)', padding: '0 12px', color: 'var(--text-muted)', fontSize: 12 }}>or sign in with email</span>
