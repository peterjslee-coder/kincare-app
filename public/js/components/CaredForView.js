              ...
              <div key={idx} className="card" style={{ marginBottom: '10px' }}>
                {isEditing ? (
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '11px', color: '#aaa', marginTop: '6px' }}>{n.createdAt ? (parseTimestamp(n.createdAt) || new Date(0)).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit' }) : ''}</div>
                  </div>
                )}
              </div>
            );
