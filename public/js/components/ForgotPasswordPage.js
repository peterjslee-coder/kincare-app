      ...
      <div className="login-container">
        <div className="login-card">
          ...
          <div style={{ textAlign: 'center', marginTop: '24px' }}>
            <a onClick={() => onNavigate('login')} style={{ color: 'var(--role-color)', cursor: 'pointer', fontWeight: '600' }}>← Back to Sign In</a>
          </div>
        </div>
      </div>
    );
    ...
    <div className="login-container">
      <div className="login-card">
        <div className="login-header">
          ...
          <p style={{ color: 'var(--text-tertiary)' }}>Enter your email and we'll send you a reset link</p>
        </div>
        <form onSubmit={handleSubmit}>
          ...
          {error && <div style={{ color: 'var(--color-error)', fontSize: '14px', marginBottom: '12px', padding: '10px', background: 'var(--bg-accent-light)', borderRadius: '6px' }}>{error}</div>}
