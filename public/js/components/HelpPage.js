...
const HelpPage = window.HelpPage = ({ currentUser, onNavigate }) => {
  ...
  return React.createElement('div', { style: { maxWidth: '800px', margin: '0 auto' } },
      ...
      categories.map(cat =>
        React.createElement('button', {
          ...
          style: {
            ...
            background: activeCategory === cat.id ? 'var(--role-color)' : 'var(--bg-surface)',
            color: activeCategory === cat.id ? 'var(--bg-surface)' : 'var(--text-secondary)',
            fontSize: '13px',
    ...
    !loading && React.createElement('div', {
      ...
      React.createElement('div', {
        ...
        React.createElement('div', null,
          React.createElement('h3', { style: { margin: '0 0 4px 0', fontSize: '16px', fontWeight: 600, color: 'var(--text-primary)' } }, 'Contact Us'),
          React.createElement('p', { style: { margin: 0, fontSize: '13px', color: 'var(--text-tertiary)' } }, 'Can\'t find what you\'re looking for? Send us a message.')
        ),
      ...
      showFeedback && (fbSubmitted
        ? React.createElement('div', { style: { textAlign: 'center', padding: '24px 0' } },
            ...
            React.createElement('div', { style: { fontSize: '16px', fontWeight: 600, color: 'var(--role-color)' } }, 'Thank you!'),
            React.createElement('div', { style: { fontSize: '13px', color: 'var(--text-tertiary)', marginTop: '4px' } }, 'Your message has been sent.')
          )
        : React.createElement('div', null,
            // Category
            React.createElement('div', { style: { marginBottom: '12px' } },
              React.createElement('label', { style: { display: 'block', fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '4px' } }, 'Category'),
              React.createElement('select', {
                value: fbCategory, onChange: (e) => setFbCategory(e.target.value),
                style: { width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #ddd', fontSize: '14px', background: 'var(--bg-surface)', boxSizing: 'border-box' },
              },
            ...
            // Description
            React.createElement('div', { style: { marginBottom: '12px' } },
              React.createElement('label', { style: { display: 'block', fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '4px' } }, 'Message'),
              ...
              fbDescription.length > 0 && fbDescription.trim().length < 10 && React.createElement('div', { style: { fontSize: '11px', color: 'var(--accent-color)', marginTop: '4px' } }, 'Please write at least 10 characters')
            ),
            // Mood
            React.createElement('div', { style: { marginBottom: '16px' } },
              React.createElement('label', { style: { display: 'block', fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '6px' } }, 'How are you feeling? (optional)'),
                ...
                [{ emoji: '😊', value: 'great' }, { emoji: '🙂', value: 'good' }, { emoji: '😐', value: 'okay' }, { emoji: '😟', value: 'bad' }, { emoji: '😡', value: 'terrible' }].map(m =>
                  React.createElement('button', {
                    ...
                    style: {
                      ...
                      background: fbMood === m.value ? 'var(--role-color-light)' : 'var(--bg-primary)',
            ...
            // Error
            fbError && React.createElement('div', { style: { padding: '8px 12px', background: 'var(--color-error-bg)', color: 'var(--color-error)', borderRadius: '8px', fontSize: '13px', marginBottom: '12px' } }, fbError),
            // Buttons
            React.createElement('div', { style: { display: 'flex', gap: '10px' } },
              React.createElement('button', {
                onClick: () => { setShowFeedback(false); setFbError(null); },
                style: { padding: '10px 16px', background: 'var(--badge-muted-bg)', color: 'var(--text-secondary)', border: '1px solid #ddd', borderRadius: '8px', fontSize: '14px', cursor: 'pointer' }
              }, 'Cancel'),
              React.createElement('button', {
                onClick: handleFeedbackSubmit,
                disabled: fbDescription.trim().length < 10 || fbSubmitting,
                style: {
                  padding: '10px 20px', background: fbDescription.trim().length >= 10 && !fbSubmitting ? 'var(--role-color)' : 'var(--border-light)',
    ...
    // ─── Dispute a Session ───
    currentUser && React.createElement('div', { style: { marginTop: 32, padding: 24, background: 'var(--bg-surface)', borderRadius: 14, boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '1px solid #e8e8e8' } },
      React.createElement('h3', { style: { margin: '0 0 8px', fontSize: 18, fontWeight: 700, color: 'var(--text-primary)' } }, '\u2696\uFE0F Dispute a Session'),
      ...
      !showDispute
        ? React.createElement('button', {
            ...
            style: { padding: '10px 20px', background: 'var(--bg-primary)', color: 'var(--text-primary)', border: '1px solid #ddd', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer' }
          }, 'File a Dispute')
        : React.createElement('div', { style: { marginTop: 8 } },
            disputeSubmitted
              ? React.createElement('div', { style: { padding: 16, background: 'var(--role-color-light)', borderRadius: 10, textAlign: 'center', color: 'var(--role-color)', fontWeight: 600 } }, '\u2705 Dispute filed. We\'ll review it shortly.')
              : React.createElement(React.Fragment, null,
                  // Session picker
                  React.createElement('div', { style: { marginBottom: 12 } },
                    React.createElement('label', { style: { display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 4 } }, 'Which session?'),
                    React.createElement('select', {
                  ...
                  // Reason
                  React.createElement('div', { style: { marginBottom: 12 } },
                    React.createElement('label', { style: { display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 4 } }, 'Reason'),
                    React.createElement('select', {
                  ...
                  // Description
                  React.createElement('div', { style: { marginBottom: 12 } },
                    React.createElement('label', { style: { display: 'block', fontSize: 13, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 4 } }, 'Description (optional)'),
                    React.createElement('textarea', {
                  ...
                  disputeError && React.createElement('div', { style: { padding: '8px 12px', background: 'var(--color-error-bg)', color: 'var(--color-error)', borderRadius: 8, fontSize: 13, marginBottom: 12 } }, disputeError),
                  React.createElement('div', { style: { display: 'flex', gap: 10 } },
                    ...
                    React.createElement('button', {
                      ...
                      style: {
                        padding: '10px 20px',
                        background: disputeSessionId && disputeReason && !disputeSubmitting ? 'var(--color-error)' : 'var(--border-light)',
                        color: 'var(--text-on-primary)', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 600,
      ...
      myDisputes.length > 0 && React.createElement('div', { style: { marginTop: 20, borderTop: '1px solid #eee', paddingTop: 16 } },
        React.createElement('h4', { style: { margin: '0 0 10px', fontSize: 15, fontWeight: 600, color: 'var(--text-secondary)' } }, 'Your Disputes'),
        ...myDisputes.map(d => React.createElement('div', {
          key: d.id,
          style: { padding: 12, marginBottom: 8, background: 'var(--bg-neutral)', borderRadius: 10, border: '1px solid #eee' }
        },
          React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' } },
            React.createElement('span', { style: { fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' } }, d.reason),
            React.createElement('span', { style: {
              fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 10,
              background: d.status === 'open' ? 'var(--color-warning-bg)' : d.status === 'resolved' ? 'var(--role-color-light)' : 'var(--bg-primary)',
              color: d.status === 'open' ? 'var(--color-warning)' : d.status === 'resolved' ? 'var(--role-color)' : 'var(--text-secondary)',
          ...
          d.admin_notes && React.createElement('div', { style: { fontSize: 13, color: 'var(--text-primary)', marginTop: 6, padding: '8px 10px', background: 'var(--color-info-bg)', borderRadius: 8 } },
