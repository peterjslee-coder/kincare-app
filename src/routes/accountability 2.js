      ...
      try {
        ...
        if (minutesPastStart >= 20 && minutesPastStart < 30
          ...
          sendPushToUser(s.caregiver_user_id, {
            ...
            body: `You started checking in for ${recipientName} but didn't finish. Tap to complete — you'll be marked as a no-show in 10 minutes.`,
            data: { type: "checkin_nudge", sessionId: s.id },
          }, "checkin_nudge").catch(() => {});
          if (_emitToUser) {
            _emitToUser(s.caregiver_user_id, "checkin_nudge", {
              sessionId: s.id,
              message: `Complete your check-in for ${recipientName} — no-show in 10 minutes.`,
            });
          }
          await db.prepare(`
        ...
        // After session start + 30 min (generous window) if still not checked in — flag as no-show
