...
- [ ] **Caretaker signup shows generic "insufficient information" error.** Instead of highlighting which fields are missing/invalid, caregiver registration just says "insufficient information." Need inline field validation with red highlights on the specific fields that need attention. *(Feedback — new)*
...
- [ ] **Profile photo upload not working for caregiver role.** peter@yourinplace.com (Carry Taker) can't upload a photo from MyAccount. Debug upload handler for caregiver role specifically. *(Feedback — Feb 23, #9)*
...
- [ ] **Dashboard "Latest" / status section.** Context-aware top section showing current status and next action for every role (e.g., "Pending background check" for new caregivers, "2 upcoming sessions" for families). *(Feedback #17 implied)*
