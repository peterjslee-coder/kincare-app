...
- [x] **"I don't know how to add a My Loved One" + undefined text:** Added gradient "Add your loved one" CTA card; fixed "undefined" name in consent banner with fallbacks.
- [x] **Getting Started checklist disappears:** Changed welcome screen condition to `isNewUser && !hasRecipient` so dashboard shows Get Started tiles after adding a care recipient.
- [x] **"Complete your profile" checked prematurely:** Strengthened `hasProfile` to require city/zip in addition to name and phone.
- [x] **Connection accepted missing name:** Activity feed title now includes the accepter's name.
- [x] **Phone number formatting:** Applied `formatPhone()` consistently in CaredForView.js and CaregiverOnboarding.js review step.
- [x] **Calendar days-of-week misalignment:** Normalized header padding and added `justifyContent: 'center'` to date cells.
- [x] **Care preferences not saving:** Simplified save handler to use PUT response directly; added error handling and toast feedback.
- [x] **Consent verification appears unexpectedly:** Added explanatory header to contextualize the verification section within the recipient view.
- [x] **Remove debug logging from jobMatching.js:** Removed console.warn/log from conflict detection block.

### Stripe Connect + Identity Cleanup + Email Fix (v1.40.0–v1.40.9)
- [x] **Video zoom control (v1.40.0):** Added pinch-to-zoom and double-tap-to-zoom for visit photos in the dashboard photo viewer.
- [x] **Install modal fix (v1.40.4):** Fixed centered install modal on mobile splash page — modal was being clipped by nav's backdropFilter stacking context. Moved modal to component root level.
- [x] **Stripe Connect error display (v1.40.5):** Show detailed Stripe error messages (including `detail` field) on caregiver payment setup screen instead of generic "failed" message.
- [x] **Stripe Connect capabilities fix (v1.40.6):** Added `card_payments: { requested: true }` alongside `transfers` in both `/connect/onboard` and `/connect/link` routes. Stripe requires `card_payments` capability unless platform has special approval.
- [x] **Stripe Connect.js polling (v1.40.7):** Added polling wait for `window.StripeConnect` async script load before calling `init()`. Prevents "Cannot read properties of undefined" error.
- [x] **Stripe Connect redirect fallback (v1.40.8):** When embedded Connect.js component is unavailable (503 from Stripe CDN), falls back to redirect-based onboarding via Stripe Account Links. Ensures caregivers can always complete payment setup.
- [x] **Remove caregiver identity verification (v1.40.9):** Removed redundant Stripe Identity verification from caregiver onboarding. Caregivers are already ID-verified through Stripe Connect (legal name, DOB, SSN, bank) + Checkr background check (full SSN, DOB). Removed: identity step from First Steps checklist, Identity Verification card from dashboard, Identity Verified badge, `identity_verified` gate from checkout flow in payments.js. Family identity verification in MyAccount.js and CareRecipients.js attestation flow preserved.
- [x] **DMARC/SPF/DKIM email fix (v1.40.9):** Fixed peter@yourinplace.com email bouncing with DMARC failure (550 5.7.26). Root cause: email routes M365 → Proofpoint Essentials (PPE Hosted) → recipient, but Proofpoint's IPs weren't in SPF and no M365 DKIM records existed. Fix: (1) Updated SPF to add `include:spf.ppe-hosted.com`, (2) Added `selector1._domainkey` and `selector2._domainkey` CNAME records in Cloudflare pointing to Microsoft's DKIM signing infrastructure, (3) Enabled DKIM signing in Microsoft Defender. M365 tenant: `NETORG20367133.onmicrosoft.com`.
...
- [x] ~~**Admin panel UX overhaul:** Redesigned in v1.41.4. Replaced 14 flat tabs with 3 grouped sections (Core, Trust & Safety, Content & Config). Added Action Required banner, unified People tab, universal search bar.~~
