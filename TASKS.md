...
- [ ] **⛔ Audit every AI generation surface for the not-medical-care acknowledgment.** Verified open: none of `src/utils/ipaiChat.js`, `src/utils/careIntelligence.js` or `IPAiInsightsCard.js` contains any disclaimer string. The age-rating answer *Medical or Treatment Information = Infrequent* is only defensible while this holds; the alternative answer triggers the Regulated Medical Device declaration. Pete's own cardinal rule, and a store-answer dependency. **P0**
- [ ] **🔒 Decide on the no-passkey impersonation bypass.** `src/routes/admin/access.js` grants a bypass when the admin has no passkey on file — it logs a warning and proceeds. Found while verifying the admin-2FA item. **Left in deliberately rather than removed: closing it could lock Pete out of Test Mode on a device with no passkey registered.** Pete's call — register a passkey everywhere and close it, or keep it and accept the gap. **P1**
