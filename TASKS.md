...
- [ ] **A2 — location is called in six places with no permission declared. P1 FOR LAUNCH.** `AreaMap`, `Caregivers`, and four sites in `CaretakerHub` (including GPS check-in) call `navigator.geolocation`, but **`NSLocationWhenInUseUsageDescription` is absent from `ios/App/App/Info.plist`** and Android's manifest declares no location permission. On iOS a missing usage string is **a hard crash, not a soft failure**. Decide: declare both with a string naming the real purpose (verifying a caregiver was at the home at check-in), or detect native and skip the geolocation path. Then **un-skip the pending test in `tests/storeReview.test.js`**.
