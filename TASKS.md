...
- [ ] **⛔ Verify GPS check-in actually works on a real iPhone before submitting.** ⚠️ **CORRECTED Aug 20** — `@capacitor/geolocation` **IS** in `package.json` at `^8.2.1` (added v1.105.67). The original wording below was written before that and has since been copied into two separate handoffs as if still true (v1.105.72 corrected it once, v1.105.113 reintroduced it). **This line is where they got it — check `package.json` before repeating it.** What is genuinely missing is unchanged: nothing calls `requestWhenInUseAuthorization()`. The Info.plist string alone may not satisfy `navigator.geolocation` in WKWebView. If it doesn't, **check-in — the app's entire safety proposition — is broken on every iPhone**, and you find out after submission. Needs Pete + a real device. **P0**
...
- [x] **Notification bell too much space on mobile.** Only shows when notifications exist but takes way too much room. Quick CSS fix — reduce size or use icon-only on mobile. *(Feedback `81cb9e47` — Pete, Mar 29)* **P2** ✅ **Closed Aug 20 — superseded, not fixed.** The bell/banner this describes no longer exists: v1.103.0 consolidated dashboard notifications into Next Up (future/act) and Activity (past/acknowledge) with the rule that nothing gets a new banner. Verified by loading the dashboard at a 500px viewport on v1.105.114 — no bell element, no wasted vertical space.
...
- [x] **Cancel requests from Schedule page.** ✅ **Shipped v1.105.117.** The blocker was never the button — the confirm-and-charge flow (fee preview, three-state copy, reason) lived inside Dashboard.js, and cloning it would have split the v1.105.15 wording fix across two files. Extracted to `public/js/components/CancelSessionModal.js`; Dashboard now renders it too, and its local copy plus the now-unreachable `handleCancel`/`cancelPreview`/`cancelLoading` state were deleted. Schedule shows Cancel on the expanded session card for future sessions in the same statuses the server accepts (`confirmed`/`pending`/`open`/`requested`). Pinned by `tests/cancelSessionModal.test.js` (6 render tests incl. the three-state rule) and a rewritten `cancellationFee.test.js` that now asserts every cancel screen routes through the shared modal. **Known remainder:** `VisitDetailModal` is still a THIRD cancel path (role-aware, `window.confirm`-based) — deliberately left alone because folding it in changes the caregiver experience. *(Feedback `3fdfdc1d` — Pete, Mar 29)* **P2**
- [x] **Passkey create/cancel buttons off-screen on mobile.** Text is right of screen and unusable. CSS overflow/wrapping issue on passkey modal buttons. *(Feedback `6706317b` — Pete, Mar 28)* **P2** ✅ **Closed Aug 20 — does not reproduce.** Measured live on v1.105.114 at a 500px viewport with the name-your-passkey row open: Create Passkey occupies x=29..387, Cancel x=395..471, both entirely inside the viewport. The row is a flex pair with `flex: 1` on Create and no fixed widths or `nowrap`, so it cannot push off-screen. Rewritten since the March report.
- [x] **Admin search text garbled on mobile.** Text in search box is overlapping and unreadable. CSS input styling issue in AdminPanel search. **Probably NOT local to AdminPanel:** the component sets `fontSize: 14` inline while `styles.css` forces `font-size: 16px !important` on every non-checkbox input under 768px. Same broad-rule-clobbers-component family as v1.105.2 — fixing it inside the component will fail. *(Feedback `56386949` — Pete, Mar 28)* **P2** ✅ **Closed Aug 20 — does not reproduce, and the diagnosis above was wrong.** The `16px !important` override does apply (measured: computed font-size 16px, min-height 44px). It just doesn't cause anything: on a fresh mount at 500px the input is 468px wide with 408px of inner width, and the placeholder needs 244px — 164px of headroom. ⚠️ **It DOES garble if you resize a desktop window down instead of loading at mobile width**, because the admin shell picks sidebar-vs-hamburger at mount and never re-picks; the 235px sidebar then survives and squeezes the content column to ~265px. That is a desktop-resize artifact, not what anyone sees on a phone — but it is why this ticket looked real. *(Measured on v1.105.114.)*
...
- [x] **v1.105.194: the caregiver's first-visit tour (Sep 12–13, 2026).** Pete: *"a guided walk-through of checking in and leaving notes and checking out. A quick click through hitting the right buttons as a tutorial"* + *"an illustrated guide to where to find things"* + *"yes on the location."* Mockup: artifact "Caregiver First-Visit Tour". `public/js/components/CaregiverTour.js`. **Eight stops:** three coachmarks on the REAL screens (Home → Up Next lit; Find Work → the first job card; Messages → the first conversation + the Care Notes tab in the bottom bar), a **practice visit** on screens shaped like check-in (briefing → "I'm here — check in" → note/photo/tick the med task → check out, ending with where the pay goes), and the five-cell **Where things live**. PRACTICE ribbon on every practice screen. **Writes nothing** — no fetch, no CareTaskSync, no localStorage (test-pinned). **The one real thing:** the practice check-in calls `navigator.geolocation.getCurrentPosition` so the permission prompt is behind her before her first real visit; a "no" is explained, not punished. **Offered** as the First Steps card's last state (`CaregiverTourCard`: offer → "Later" folds to one line → done = map card until first real check-out); replayable from **Help → "Show me around again"**. Remembered on the account (`ui_prefs` `tour.caregiver.done` / `.later`, new `window.__setUiPref`). Hosted in `app.js` above everything (`tourOpen`, `window.__startCaregiverTour`), z 1300 — above the PWA install banner, which it hides for its lifetime. Anchors: `data-tour="up-next" | "calendar" | "jobs" | "job-first" | "conversations" | "conversation-row" | nav-*`. Walked all eight stops on staging as Maria at phone size. 13 tests (react-dom/server renders + pins). Bottom line for Tina: the moment her First Steps close she is offered it; her Android will ask for location during the practice check-in.
- [x] **v1.105.193: a locked Find Work says why (Sep 12, 2026).** Tina: everything on her path done except preferences and availability, `onboarding_complete` still 0, and the Find Work button was a dead tap — greyed, no sentence, no response. Now all three Find Work buttons (sidebar action, sidebar link, bottom bar) toast "Find Work unlocks when you finish: What you're happy doing, When you're free, and your rate." and go to Home, where the First Steps card lists them. The hub exposes `window.__caregiverFirstStepsLeft` (labels of `todo` route items). **Found on the way:** `window.__showToast` had two callers in the push handlers (`utils.js`) and no definition — the "new notification" toast had never once shown. Defined in `ToastProvider`.
- [x] **v1.105.192: Sep 12 feedback pull — 8 items, and the first outside caregiver (Tina Houston) (Sep 12, 2026).** Read Tina's record through Pete's session before anything: she never clicked the known-caregiver link — she found the app and signed up on her own with the invited email; the invite sat `pending` while Pete vouched (admin, "I've interviewed her in person") and assigned her by hand. Her client is current (1.105.191, iPhone — not Android after all), Stripe complete, `onboarding_complete` 0, no legal name / DL number on file (she has not done wizard screen 4). **No data was changed by me.**
  - **"I would expect her to automatically show up as an assigned caregiver"** → `claimPendingByEmail`: a pending, unexpired known-caregiver invite whose email matches a caregiver account is claimed by that account — at signup (`auth.js` register), at profile creation, and on every caregiver dashboard load (idempotent). Gate row, assignment, phone, leader told. For Tina this fires on her next dashboard open; her existing admin vouch means no duplicate gate row.
  - **"Am I supposed to waive the background check fee?"** → No. The vouch covers your family; the fee only matters if she wants other families' work. The admin fee row now says exactly that when a vouch exists. (Her `background_check_paid` is already 1 — harmless.)
  - **"Why does it show drivers license: no?"** → that flag was `dl_number && dl_state`, the NUMBER typed on wizard screen 4, unrelated to the approved licence photo. Relabelled "Licence number typed" and it says "photo approved, number not entered" when that is the case.
  - **Daniel's Done "freezes"** → the circle did nothing until the round trip and the re-fetch returned. Now optimistic: row turns done with "Saving…" at once, a second tap is ignored, failure reverts with a toast.
  - **"hand tonight" on an 8 AM task** → "Hand this task to Daniel — not completed."
  - **Recurring request = many reminders** → the Awaiting Caregiver card folds a series into one card on the soonest occurrence ("Weekly series — no caregiver yet. Whoever accepts takes the whole series (5 more upcoming)"); its Cancel cancels the series via the existing `DELETE /api/sessions/recurring/:groupId`. Next Up never listed unfilled requests, so nothing else to fold.
  - **Triaged, not built:** (1) fold due care tasks into the caregiver's check-in instructions; (2) a guided first-visit walkthrough (check in → note → check out). Both in Features — Up Next below.
- [x] **v1.105.191: hand tonight's task to someone, and remove a task for good (Sep 11, 2026).** Pete: *"I want a way to assign the task we're waiting on (mom's meds tonight) to Daniel. or sara. it defaults to me and if I open it and select dan, my only option in the 'needs you' pane is to complete the task or skip...no 'save and update'."* The sheet's picker was "who DID it"; picking a team member now also offers **"Hand tonight to Daniel — not done yet"**. Occurrence-level `care_task_occurrences.assigned_user_id` (MIGRATIONS_V2 `029`); effective person = `COALESCE(occurrence, task)` in `/today`, the poller's due push, and Needs-you (`attention.js`). The task's default stays Pete's — tomorrow it is his again; `userId: null` hands it back. Daniel gets "It's your turn tonight — Pete handed you something due at 7:00 PM for Betty" (no med name on the lock screen, v1.105.39). `POST /api/care-tasks/occurrences/:id/assign`, `canCheckOff`, 409 once checked off. **And** *"i still can't cancel tasks"*: `DELETE /api/care-tasks/:id` had existed since Phase 1 with nothing on screen calling it — Pause was the only way to stop a finished antibiotics course, and it kept the row. **Remove** button beside Pause, confirm says history stays. 7 integration + 4 source tests.
- [x] **v1.105.190: the Care Notes tab exists on the phone (Sep 11, 2026).** Pete: *"julia still can't see care notes"* — third report. Checked the data through Pete's session first: Julia's share row carries `read_notes`, `/api/notes/mine/recipients` lists Betty for her, and her client was on 1.105.188 that morning. Then the code: v1.105.153 built the tab and v1.105.184 fixed when it opens — both in `getNavItems`, the **desktop sidebar**. The caregiver **bottom bar** (`getBottomNavItems`) was a fixed four. On the iPhone she uses, the tab had never existed once. Two fixes shipped against a door on the other building. Now the bottom bar carries 📝 Care Notes on the same server answer, before Account (five items, same as the family bar). `tests/careNotesTabOnThePhone.test.js`. ⚠️ **Wants Julia to open the app and see it — she needs the .190 bundle, so a quit-and-reopen once.** Lesson for the rule file: a caregiver-facing nav change is not done until BOTH `getNavItems` and `getBottomNavItems` carry it — nobody real uses the sidebar.
- [x] **v1.105.189: the family's Observations & Notes card is one timeline, in time order (Sep 11, 2026).** Pete: *"Daniel's notes are waaaay out of date order?"* Verified against his real record via his own session before changing anything: the API returned notes newest-first and visits newest-first, and the card rendered **every visit, then every note** — Daniel's three notes from that morning sat under visits from Sep 10, 9, 8, 7, 5, 4. Same shape the caregiver view fixed in v1.105.156; the family view never got it. Now `noteTimeline` merges both, sorted by instant; "newest five, then ask" counts the merged list; the push deep-link expander uses the merged index. Visits keep their FAMILY VISIT label (v1.105.38 — never blended). **Also:** Daniel's 14:27 note existed **twice**, one second apart — Enter + button before the disabled state painted. `handleAddNote` is now locked by a ref, not state. `tests/careProfileTimeline.test.js` (6); three older pins updated because they pinned the stacked shape.
  - **Feedback inbox swept (9 items):** emoji/reactions look (→ .170–.175), "Due invalid date" (→ `invalidDateAndNoteDestination`), same-emoji-on-every-entry (→ .170), notes in Activity incl. own (→ .182), collapsible sections (→ .171), Julia vouch candidates (→ .63), iPAi reading care-team messages (⏸️ suspended by Pete, stays in P1). **Still open: "Julia's notes still not fixed"** — ambiguous, asked Pete which of three it is; kept in FEEDBACK_INBOX.md (local, untracked).
- [x] **v1.105.188: "I want a notification when she's joined. will i get it?" (Sep 10, 2026).** Partly, was the honest answer: .186 pushed at ACCEPT ("Carol is setting up") but nothing when she became bookable. Now `notifyIfReadyToBook` (`src/utils/knownCaregivers.js`) fires "**Carol is ready to book** — set up to be paid, licence photo in" to the leader (push `page:"caregivers"` + activity feed) from whichever lands last: Stripe complete (webhook + status route in `payments.js`) or the licence photo (`caregiveronboarding.js` verify-id). Once — the invite is claimed `accepted → ready` with a conditional UPDATE so racing call sites cannot double-notify. `progressFor` moved to the util. Integration test covers not-yet / fires / once / still listed.
- [x] **v1.105.187: app updates apply themselves at a safe moment, behind an "App updated — back in a sec" card (Sep 9, 2026).** Pete opened the app after .186 deployed and every screen switch spun for a minute — old bundle against the new server, with "Update ready — tap to refresh" sitting there unnoticed. A full quit fixed it. Pete: *"is there a way to force the app to restart when there's an update... or a screen that says 'App updated, reloading your data... back in a sec!'"* Both, with one rule from history (auto-reload on arrival broke dashboards pre-v1.57.14): **never mid-work.** (1) Update already waiting when the app opens → apply now behind the card. (2) Update lands while open → pill stays, AND it applies on the next screen change or foreground return **if** no form has text and no `.modal-overlay` is open (`swIsSafeMoment`). (3) Typing/booking → wait. Lives in `public/index.html` (the bundle is the stale thing); app.js calls `window.__swApplyPendingUpdate()` on `currentPage` change; 8s fallback reload if `controllerchange` never fires. `tests/swUpdateSafeMoment.test.js` (11) runs the inline script against a fake DOM. The worker itself is unchanged — still only `skipWaiting` when told.
- [x] **v1.105.186: add a caregiver you already know (Sep 8, 2026).** Pete: *"I find someone interested in the wild. I get their name and number and email, and the next thing they get is an email to finish setting up their account."* Plan: `docs/Known_Caregiver_Invite_Plan_2026-09-08.md`. Mockup: artifact "Known Caregiver Invite".
  - **Posture, decided by Pete — NOT a vouch, not a lawyer item:** *"inplace doesn't control who gets to work for families in the event they only use us to coord their own care."* The family brought the caregiver; every surface says so in plain words ("Your caregiver · no background check", "Pete brought you in"); the word "vouch" never reaches a screen on this path (pinned by `tests/knownCaregiverInvite.test.js`). The mechanism underneath is the existing per-family gate row `bg_admin_vouches` with `note='family-brought'`, keyed on the recipient's **OWNER** (what `sessions.js` checks), `vouched_by` = the leader.
  - **Family side:** Caregivers tab → card "Add someone you already know" above the tabs; name + email + optional phone (stored on their profile, never texted — SMS stays blocked on TCPA/10DLC); an "Invited" list under Assigned with honest states (sent / expired / "2 of 4 done") and Resend / Withdraw. Leader-only (`recipientIfLeader` — owner or care-team leader), 5 open invites per leader, one open invite per email, 14-day link.
  - **Caregiver side — the short path:** `onboardingRoute.js` gets a `familyOnly` fact; nine of the thirteen become `optional` (present, never counted, never current), `remaining` starts at four. Wizard screens 1–3 → 8 → 9; screen 4 (safety-check details, SSN) is never visited. Hero: "Pete Lee added you as Betty's caregiver". The route item `paperwork` is now labelled **"A few quick details"** everywhere (Pete: "we promise paperwork three times"). The background-check disclosure on the short path says when the check applies instead of "for all caregivers" — ⚠️ counsel-adjacent, flag further rewording. Dashboard: optional items drawn as one grey "Whenever you like" line; the First Steps card closes when HER four are done so Find Work unlocks.
  - **Plumbing:** `platform_invites` + `kind`, `care_recipient_id`, `invited_name`, `phone` (MIGRATIONS_V2 `028`). `POST/GET /api/known-caregivers`, `/:id/resend`, `DELETE /:id`. `/api/platform-invites/info` returns `kind`, `recipientFirstName`, name parts, `phone`, `existingAccount`; `accept-invite` writes the gate row + phone and tells the **leader** (push `page:"caregivers"`), not the admins. Assignment is written when the profile first exists (`fulfillPendingForUser` from `POST /api/caregivers/profile`). Existing caregiver account → "Sign in" screen, token parked in `localStorage.pendingPlatformInvite`, accepted by app.js on session restore or login.
  - **Tests:** 18 source pins + 10 integration (real Postgres: leader gate, 409 on duplicate, /info shape, gate row on owner, idempotent, assignment after profile, progress 2 of 4, resend/withdraw, family-account email refused). 118 suites / 1722 green; 36 integration suites / 282 green.
  - **Not verified yet:** the email as rendered in a real client; the wizard on a phone (step 3 → 8 jump, hero copy); Pete's Caregivers tab on prod with Betty. Staging first.
...
- [ ] **Care tasks at check-in.** (Pete, 9/12) When a caregiver checks in and a care task is due inside the visit window (medication, a reminder, an appointment needing transport), fold it into the check-in instructions so the visit and the task are one screen. Server has everything: `care_task_occurrences` due_at vs session window. Decide: who gets credit when the caregiver checks it off (the who-did-it picker already exists).
- [ ] **First-visit walkthrough for caregivers.** (Pete, 9/12) A click-through before the first session: check in → leave a note → check out, on the real buttons. `CareStoryWalkthrough.js` exists as a pattern; trigger = caregiver with zero completed visits and a confirmed session within 24h.
